...
object SmallLanguageParsers extends STokenParsers with StrongParsers with ImplicitConversions {
  ...
  def validate(rawResult: ParseResult[List[Definition]]): ParseResult[List[Definition]] = {
    ...
    val fArity = mutable.Map[String, Int]()
    val gArity = mutable.Map[String, Int]()

    // remembering the first arity from definitions
    for (definition <- rawProgram) definition match {
      case FFunction(name, args, _) =>
        if (!fArity.contains(name) && !gArity.contains(name))
          fArity += (name -> args.size)
      case GFunction(name, _, args, _) =>
        if (!fArity.contains(name) && !gArity.contains(name))
          gArity += (name -> (args.size + 1))
    }

    val cInfo = mutable.Map[String, Int]() // arity for constructor
    val fInfo = mutable.Map[String, Int]() // arity for f-function
    val gInfo = mutable.Map[String, Pair[Int, mutable.Set[String]]]() // name -> (arity, constructor names)
    ...
    def validate(definition: Definition): ParseResult[Definition] =  {
      ...
      val fVars = mutable.Set[Variable]()
      ...
      def validateTerm(t: Term): ParseResult[Term] = t match {
        ...
        case c@Constructor(name, args) =>
          if (cInfo.contains(name) && cInfo(name) != args.size)
            error(name + " is already defined as constructor with arity " + cInfo(name), c)
          else {
        ...
        case fc@FCall (name, args) =>
          ...
          if (fArity.contains(name)) {
            ...
            // we assume that there is a call to undefined f-function
            fArity(name) = args.size
          }
          val proceededArgs = new ListBuffer[Term]
          for (arg <- args) {
      ...
      definition match {
        case FFunction(name, args, rawTerm) =>
          ...
          validateTerm(rawTerm) match {
            case Success(term: Term, _) => Success(FFunction(name, args, term), null)
            case r: NoSuccess => r
          }
        ...
        case GFunction(name, arg0, args, rawTerm) =>
          ...
          if (fInfo.contains(name)) return error(name + " is already defined as f-function " + name, definition)
          // 2. fixed arity and constructor uniqueness - also constructor global uniqueness
          val cName = arg0.name
          ...
          if (gInfo.contains(name)) {
            ...
            gInfo(name) = (args.size + 1, mutable.Set(cName))
          }
          ...
          validateTerm(rawTerm) match {
            case Success(term, _) => Success(GFunction(name, arg0, args, term), null)
            case r: NoSuccess => r
          }
      }
    }
    ...
    def error(msg: String, pos: Positional) =
      new SError(msg, pos, rawResult.next)

    val ds = new ListBuffer[Definition]()
    for (definition <- rawProgram) {
      validate(definition) match {
        case r: NoSuccess => return r
  ...
  class SError(override val msg: String, val pos: Positional, next: Input) extends Error(msg, next) {
...
class STokenParsers extends StdTokenParsers {
  ...
  def uident: Parser[String] =
    elem("uidentifier", x => x.isInstanceOf[Identifier] && x.chars.charAt(0).isUpper) ^^ (_.chars)

  def lident: Parser[String] =
    elem("lidentifier", x => x.isInstanceOf[Identifier] && x.chars.charAt(0).isLower) ^^ (_.chars)
}