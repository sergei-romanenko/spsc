...
class BasicScpTest < Test::Unit::TestCase
  include SLL
  include SLLParser
  include Algebra
  include BasicProcessTreeBuilder
  def setup()
  ...
  def buildPrTree1(prog, e)
    ...
    BasicProcessTreeBuilder.build(nameGen, 1, prog, e)
  end
  ...
  def buildPrTree(prog, e)
    nameGen = NameGen.new("v", 100)
    BasicProcessTreeBuilder.build(nameGen, 100, prog, e)
  end
