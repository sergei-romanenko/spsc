...
#
    ...
    def test201PrTrCtr1(self):
        "PrTrCtr1"
        self.buildPrTree1OK("", "S(Z)",
                            "{0:(S(Z),None,None,[1]),1:(Z,None,0,[])}")

    def test202AddS_Z1(self):
        "AddS_Z1"
        self.buildPrTree1OK(self.pAddAcc, "gAddAcc(S(Z), Z)",
                            "{0:(gAddAcc(S(Z),Z),None,None,[1]),1:(gAddAcc(Z,S(Z)),None,0,[])}")

    def test203AddAB1(self):
        "AddAB1"
        self.buildPrTree1OK(self.pAdd, "gAdd(a, b)",
                            "{0:(gAdd(a,b),None,None,[1,2]),1:(b,a=Z,0,[]),2:(S(gAdd(v10000,b)),a=S(v10000),0,[])}")

    def test204AddAdd1(self):
        "AddAdd1" 
        self.buildPrTree1OK(self.pAdd, "gAdd(gAdd(a,b),c)",
                            "{0:(gAdd(gAdd(a,b),c),None,None,[1,2]),1:(gAdd(b,c),a=Z,0,[]),2:(gAdd(S(gAdd(v10000,b)),c),a=S(v10000),0,[])}")

    def test301AddS_Z(self):
        "AddS_Z"
        self.buildPrTreeOK(self.pAddAcc, "gAddAcc(S(Z), Z)",
                            "{0:(gAddAcc(S(Z),Z),None,None,[1]),1:(gAddAcc(Z,S(Z)),None,0,[2]),2:(S(Z),None,1,[3]),3:(Z,None,2,[])}")

    def test303AddAB(self):
        "AddAB"
        self.buildPrTreeOK(self.pAdd, "gAdd(a, b)",
                            "{0:(gAdd(a,b),None,None,[1,2]),1:(b,a=Z,0,[]),2:(S(gAdd(v10000,b)),a=S(v10000),0,[3]),3:(gAdd(v10000,b),None,2,[])}")

    def test304AddAdd(self):
        "AddAdd" 
        self.buildPrTreeOK(self.pAdd, "gAdd(gAdd(a,b),c)",
                            "{0:(gAdd(gAdd(a,b),c),None,None,[1,2]),1:(gAdd(b,c),a=Z,0,[3,4]),3:(c,b=Z,1,[]),4:(S(gAdd(v10001,c)),b=S(v10001),1,[5]),5:(gAdd(v10001,c),None,4,[]),2:(gAdd(S(gAdd(v10000,b)),c),a=S(v10000),0,[6]),6:(S(gAdd(gAdd(v10000,b),c)),None,2,[7]),7:(gAdd(gAdd(v10000,b),c),None,6,[])}")
