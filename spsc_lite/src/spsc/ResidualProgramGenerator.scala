  ...
  private def walk(n: Node): Term = if (n.fnode == null) n.expr match {
    ...
    case call: Call =>
      if (n.outs(0).branch != null) {
        sigs(n) = Signature(rename(call.f, false, false), vars(call))
        for (e <- n.outs) defs += GFun(sigs(n).name, e.branch.pat, vars(call).tail, walk(e.child))
        GCall(sigs(n).name, vars(call))
      } else if (tree.leafs.exists(_.fnode == n)) {
        sigs(n) = Signature(rename(call.f, n == tree.root, true), vars(call))
        defs += FFun(sigs(n).name, sigs(n).args, walk(n.children(0)))
        FCall(sigs(n).name, vars(call))
