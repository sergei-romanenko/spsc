	...
	return {
		...
		drive: function (e) {
			switch (e.kind) {
				...
				case 'FCall':
					...
					return [[Algebra.apply_subst(f_rule.exp, map), null]];
				case 'GCall':
					var arg1 = e.args[0];
					switch (arg1.kind) {
						case 'Constructor':
							...
							return [[Algebra.apply_subst(g_rule.exp, map), null]];
						case 'Variable':
							var res = [];
							...
							for (var i = 0; i < g_rules.length; i++) {
								...
								var map = {};
								map[arg1.name] = fc;
								var x = this.drive(Algebra.apply_subst(e, map));
								...
								var gc = Lang.gcall(e.name, [inner_dr[0]].concat(e.args.slice(1)
									.map(function (arg) {
										if (inner_dr[1]) {
											...
											return Algebra.apply_subst(arg, map);
										} else {
											return arg;
										}
									})));
		...
		fresh_pattern: function (p) {
			...
			for (var i = 0; i < p.args.length; i++) {
				new_args.push(Algebra.fresh_var());
			}
				...
				switch (b.exp.kind) {
					...
					case 'GCall':
						...
						for (var i = 0; i < ancs.length; i++) {
							var _a = ancs[i];
							if (_a.exp.kind == b.exp.kind && Algebra.instance_of(_a.exp, b.exp)) {
								a = _a;
								break;
							}
						}
						if (a) {
							var map = Algebra.match_against(a.exp, b.exp);
							var bindings = [];
							for (var n in map) {
...
function supercompiler(program) {
		...
		while (t.get_unprocessed_leaf()) {
			...
			switch (b.exp.kind) {
				...
				case 'GCall':
					...
					if (a) {
						if (Algebra.instance_of(a.exp, b.exp)) {
							this.abs(t, b, a);
	...
	s.split = function (t, n) {
		...
		var l = Lang.let_(Algebra.replace_args(n.exp, fresh_vars), bindings);
		t.replace(n, l);
	};
