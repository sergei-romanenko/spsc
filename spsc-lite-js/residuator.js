...
function residuator(tree) {
	...
	return {
		residuate: function () {
			...
			var new_program = Lang.program(defs);
		...
		walk: function (node) {
			...
			switch (exp.kind) {
				...
				case 'Constructor':
					...
					return Lang.constructor(exp.name, args);
				case 'FCall':
				case 'GCall':
		...
		walkCall: function (node, name, args) {
			...
			if (node.children[0].contraction) {
				...
				for (var i = 0; i < node.children.length; i++) {
					...
					defs.push(Lang.grule(gname, cn.contraction[1], vs.slice(1), this.walk(cn)));
				}

				return Lang.gcall(gname, vs);
				...
				if (func_node) {
					...
					var sig = [fname, vs];
					node.sig = sig;
					defs.push(Lang.frule(fname, vs, this.walk(node.children[0])));
					return Lang.fcall(fname, vs);
				} else {
