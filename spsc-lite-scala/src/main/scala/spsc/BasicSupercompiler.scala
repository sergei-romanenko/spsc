...
class BasicSupercompiler(p: Program) {
  ...
  def applyContr(c: Contraction)(term: Term): Term = {
    if (c == null)
      ...
      val subst = Map(c.v -> Ctr(c.pat.name, c.pat.args))
  ...
  def driveExp(expr: Term): List[(Term, Contraction)] = expr match {
    ...
    case FCall(name, args) =>
      List(
        (applySubst(Map(p.f(name).args.zip(args): _*), p.f(name).term), null)
      )
    case GCall(name, Ctr(cname, cargs) :: args) =>
      ...
      List(
        (
          applySubst(
            Map((g.p.args ::: g.args) zip (cargs ::: args): _*),
            g.term
          ),
          null
        )
    ...
    case GCall(name, args) =>
      driveExp(args(0)) map { case (k, v) =>
        (GCall(name, k :: (args.tail.map(applyContr(v)))), v)
      }
  ...
  def buildProcessTree(e: Term): Tree = {
    ...
    while (t.leaves.exists { !_.isProcessed }) {
      ...
      t = b.ancestors.find(a =>
        isFGCall(a.expr) && instOf(b.expr, a.expr)
      ) match {
        case Some(a) =>
          t.replace(b, Let(a.expr, matchAgainst(a.expr, b.expr).toList))
